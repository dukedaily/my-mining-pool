...
export const registerContractInJsonDb = async (contractId: string, contractInstance: Contract, args?: (string | string[])[]) => {
    ...
    if (MAINNET_FORK || (currentNetwork !== 'hardhat' && !currentNetwork.includes('coverage'))) {
        ...
        console.log(`------ duke -----: ${contractId}  存储完毕!`);
        console.log(`\n******`);
    }
    ...
    if (args === undefined) {
        ...
        await getDb()
            .set(`${contractId}.${currentNetwork}.${args}`, {
                address: contractInstance.address,
                // deployer: contractInstance.deployTransaction.from,
            })
            .write();
...
export const withSaveAndVerify = async <ContractType extends Contract>(
    ...
    // console.log("in withSaveAndVerify call registerContractInJsonDb");
