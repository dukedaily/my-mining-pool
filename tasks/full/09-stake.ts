...
task("full:stake", "initialize ")
    ...
    .setAction(async ({ verify }, localDRE) => {
        ...
        console.log("acc0:".yellow, account0.address);
        console.log("acc1:".yellow, account1.address);
        console.log("acc2:".yellow, account2.address);
        console.log("acc3:".yellow, account3.address);
        console.log("acc4:".yellow, account4.address);

        let youTokenSymbol = 'You'
        let YouToken = await getTokenYouTest(youTokenSymbol)

        let x = await YouToken.balanceOf(account0.address)
        console.log("you balance:", x.toString(), "details:", (await YouToken.decimals()).toString())

        // let factoryV2 = await getYouSwapFactoryV2()
        ...
        console.log("createBigNumber18(1):", createBigNumber18(1));
        ...
        // let powerRatio = createBigNumber18(1)
        let powerRatio = 1
        ...
        let rewardTotals = [createBigNumber6(50)] //提前结束
        ...
        // console.log('创建pool3');
        // await factoryCreator.createPool(0, stakeToken, BUSDToken.address, enableInvite, poolParams3, rewardTokens, rewardTotals, rewardPerBlocks)

        let myFactory = await factoryCreator.getMyFactory(account0.address)
        console.log("myFactory:", myFactory.toString());

        let youBalance = await factoryCreator.getBalance()
        console.log("factoryCreator you balance:", youBalance.toString());

        let allFactories = await factoryCreator.getAllFactories()
        console.log("allFactories:", allFactories.toString());

        let myFactoryRewardYouBalance = await YouToken.balanceOf(myFactory)
        console.log("myFactoryRewardYouBalance:", myFactoryRewardYouBalance.toString());
        ...
        // await inviteIns.connect(account1).inviteBatch([account0.address])
        // await inviteIns.connect(account2).inviteBatch([account1.address])
        ...
        console.log('balanceYouToken111:', balanceYouToken.toString());

        balanceYouToken = await YouToken.balanceOf((await myFacoryInstance.core()).toString())
        console.log('balanceYouToken222:', balanceYouToken.toString());

        console.log("************** account0 操作质押stake!*****************".red);
        await YouToken.approve(myFacoryInstance.address, createBigNumber18(100000000))
        ...
        await WBTCToken.approve(myFacoryInstance.address, createBigNumber6(100000000))
